#
# v4 goes back to original temporal parameters
# study 500 ms --> blank 500 ms --> mask 500 ms --> probe
# but presents the annulus all the way through until it changes color in the probe
#
# files suffixed SPA for Single Probe Annulus

# odd numbered participants see
# SameKey = 'z'; ChangeKey = 'm'
# even numbers see the opposite mapping
...
def trialSeq(block, showLocs = False):
    ...
    for thisTrial in block:
        ...
        # set annulus color
        probeCir.setColors(annulusColor)
        ...
        if showLocs:
            # fill study locations in irrelevant color for probe
            for s in range(N):
                trialStim[s].setLineColor(annulusColor)
                trialStim[s].setFillColor(annulusColor)
            
            [trialStim[i].draw() for i in range(N)]
        
        # present probe
        probeCir.setColors(colours[test])
        # testStim = visual.Rect(win, width = stimSize, height = stimSize, pos = testLoc, lineColor = colours[test], fillColor = colours[test])
        probeCir.draw()
...
def main():
    ...
    global SameKey, ChangeKey
    if (expInfo['Participant'] - 1) % 2 == 0:
        SameKey = 'z'
        ChangeKey = 'm'
    else:
        SameKey = 'm'
    ...
    fileName = 'Participant' + str(expInfo['Participant']) + '_SPA' # + expInfo['dateStr']
    ...
    # probe annulus
    elementLocs = [[numpy.sin((i*numpy.pi/180))*cirRadius, numpy.cos((i*numpy.pi/180))*cirRadius] for i in range(360)]
    global probeCir
    probeCir = visual.ElementArrayStim(win, units = 'deg', fieldPos = [0,0], fieldShape = 'circle', nElements = 360, 
    xys= elementLocs, sizes = .5, elementMask = 'circle', elementTex = 'none', texRes = 400, phases = 1)
